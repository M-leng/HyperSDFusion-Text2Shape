...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        samples, intermediates = self.ddim_sampling(model_kwargs, size,
                                                    callback=callback,
    ...
    @torch.no_grad()
    def ddim_sampling(self, model_kwargs, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(img, model_kwargs, ts, index=index, use_original_steps=ddim_use_original_steps,
    ...
    @torch.no_grad()
    def p_sample_ddim(self, x, model_kwargs, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None, mm_cls_free=False):
        b, *_, device = *x.shape, x.device

        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            e_t, _ = self.model.apply_model(x, t, **model_kwargs)
